...
* **按 PDF 隔离本地历史**：以 PDF 附件 (`pdf_<attachmentID>`) 维度在本地保存对话历史，不同文献间互不干扰。数据保存在 Zotero 数据目录下的 PaperPilot 目录中。
...
* 全文精读会根据 PDF 文本长度选择单次全文分析或分段汇总策略。较长论文可能需要更多时间，并可能受到模型上下文长度、API 限流和网络状况影响。
...
* **本地管理密钥**：API Key 由用户在本地自行配置，插件代码与发布包中不包含任何内置 API Key。
* **第三方接口数据传输**：使用在线翻译或 AI 功能时，选中的文本、局部上下文或论文文本会直接发送至用户所选择的外部服务提供商。
* **全文精读传输范围**：在全文精读模式下，可能会将较大篇幅或全部提取文本传输至已配置的 AI 服务商。
* **敏感数据提示**：处理敏感、未公开或涉密文献时，请根据自身所在机构的数据安全政策决定是否启用外部在线服务。
